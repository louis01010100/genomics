...
The fixture under ``cram_depth/fixture/`` is built by ``build_fixture.sh`` from real
1000G GRCh38 CRAMs: tiny windows are sliced onto a mini-reference so that four
samples (2 male, 2 female) exercise every edge case in a few thousand positions:
...
FIXTURE = HERE / 'cram_depth' / 'fixture'
...
def test_autosomes_schema_and_contigs(outputs):
    ...
    by_contig = {}
    for r in rows:
        by_contig.setdefault(r['chrom'], 0)
        by_contig[r['chrom']] += 1
...
def test_sex_schema_and_contigs(outputs):
    ...
    contigs = {r['chrom'] for r in rows}
    assert contigs == {'chrX', 'chrY'}
...
def test_chrY_male_present_female_near_zero(outputs):
    ...
    assert male > 5.0
    assert female < male / 5          # females essentially lack chrY
    assert zero_female > 0            # -aa emits zero-depth positions
